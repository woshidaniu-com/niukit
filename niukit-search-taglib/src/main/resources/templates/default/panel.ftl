...
(function($){
	...
	$.fn.extend({
		searchInit:function(){
			var $panel = $(this);
			//绑定输入框回车事件
			$(":input[name=search_blurValue]",$panel).bind("keyup",function(e){
				if (e.keyCode == 13){
					$(":button[name=search_button]",$panel).click();
				}
			});
			//清空输入框内容
		    ...
		    // 绑定添加条件，一个时间弹出框,仅支持laydate 1
		    $(":button[name=addSingleDateBtn]",$panel).click(function () { // 绑定添加条件，一个时间弹出框
		        var str = '';
		        var strHtml = '<li class="selected"><a class="cur transition-common"></a></li>';
		        var $fg = $(this).parent('.form-group');
		        var sv = $fg.find('input:first').val();
		        ...
		        str = sv + '~' + iv;
		        var $strHtml = $(strHtml);
		        $strHtml.find('a').attr('title', sv);
		        $strHtml.find('a').html(sv + ' <i class="fa fa-remove close-icon"></i>');
		        $strHtml.data("begin",sv);
		        $strHtml.data("end",iv);
		...
		getSearchMap:function(){
			...
			map["searchModel.inputSqlType"]=sqlType;
			return map;
		},
