...
;(function($){
	...
	$.fn.includes = function(urls,options){
        ...
        //递归执行加载
        function proxy(dfd_all,includes,i){
        	return $.when($.Deferred(function(dfd){
        		includes[i].call($this,dfd,urls[i]);
            }).promise()).always( function(){ 
            	if(i < includes.length){
            		//加载下一个文件
            		proxy(dfd_all, includes, i + 1);
            	}else{
            		// 改变deferred对象的执行状态为：已完成
            		dfd_all.resolve(); 
            	}
        ...
        //执行延时对象
       return $.when($.Deferred(function(dfd_all){
        	try {
        		var includes = [];
                for (var i = 0; i < urls.length; i++) {
                	//预定义加载函数
                	includes[i] = function(dfd,url){
                		console.log("url:" + url);
                		$($this).include(url,options).fail(function(){
                			//是否忽略错误项
        			   	   	if(options.ignore && options.ignore != true){
        			   	   		// 改变Deferred对象的执行状态为：已失败
        			   	   		dfd.reject(); 
	...
	$.fn.loadResources = function(urls,options){
        ...
        //递归执行加载
        function proxy(dfd_all,resources,i){
    		return $.when($.Deferred(function(dfd){
    			if(typeof resources[i] != 'undefined'){
    				resources[i].call($this,dfd,urls[i]);
    			}else{
    				// 改变deferred对象的执行状态为：已完成
    				dfd.resolve(); 
    			}
       ...
       return $.when($.Deferred(function(dfd_all){
        	try {
        		var resources = [];
                for (var i = 0; i < urls.length; i++) {
                	resources[i] = function(dfd,url){
                		//console.log("url:" + urls[i]);
                		console.log("url:" + url);
            			$($this).loadResource(url,options).fail(function(){
                			//是否忽略错误项
        			   	   	if(options.ignore && options.ignore != true){
        			   	   		// 改变Deferred对象的执行状态为：已失败
        			   	   		dfd.reject(); 
