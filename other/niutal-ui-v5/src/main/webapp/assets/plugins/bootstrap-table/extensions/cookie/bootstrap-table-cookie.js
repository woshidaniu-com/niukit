...
(function ($) {
    ...
    var setServerCookie = function(url,tableid){
    	$.get(url,
    			{cookieIdTable:tableid,cookie_cache_by_bootstrap_table:'true',send_by_bootstrap_table:'true'},function(data){});
    };
    ...
    var getServerCookie = function(url,tableid, callback){
    	$.ajaxSetup({async:false});
    	$.get(url,
    			{cookieIdTable:tableid,cookie_query_by_bootstrap_table:'true',send_by_bootstrap_table:'true'},function(data){
    		callback(data);
    	});
    ...
    BootstrapTable.prototype.initCookie = function () {
        ...
        //如果本地浏览器没有cookie信息,需要查询服务器
        if(		sortOrderCookie == undefined &&
        		sortOrderNameCookie == undefined &&
        		pageListCookie == undefined &&
        		pageNumberCookie == undefined &&
        		columnsCookie == undefined &&
        		searchTextCookie == undefined
        		){
        	 var $thisOpts = this;
        	 getServerCookie(this.options.cookieUrl, this.options.cookieIdTable, function(data){
        		 if(data){
        			 sortOrderCookie = data[$thisOpts.options.cookieIdTable + '.' + cookieIds.sortOrder];
                     sortOrderNameCookie = data[$thisOpts.options.cookieIdTable + '.' + cookieIds.sortName];
                     pageNumberCookie = data[$thisOpts.options.cookieIdTable + '.' + cookieIds.pageNumber];
                     pageListCookie = data[$thisOpts.options.cookieIdTable + '.' + cookieIds.pageList];
                     
                     var data_column = data[$thisOpts.options.cookieIdTable + '.' + cookieIds.columns];
                     columnsCookie = data_column == undefined ? undefined : JSON.parse(decodeURIComponent(data_column));
                     searchTextCookie = data[$thisOpts.options.cookieIdTable + '.' + cookieIds.searchText];
        		 }
        		 //sortOrder
        		 $thisOpts.options.sortOrder = sortOrderCookie ? sortOrderCookie : $thisOpts.options.sortOrder;
    	         //sortName
        		 $thisOpts.options.sortName = sortOrderNameCookie ? sortOrderNameCookie : $thisOpts.options.sortName;
    	         //pageNumber
        		 $thisOpts.options.pageNumber = pageNumberCookie ? +pageNumberCookie : $thisOpts.options.pageNumber;
    	         //pageSize
        		 $thisOpts.options.pageSize = pageListCookie ? pageListCookie === $thisOpts.options.formatAllRows() ? pageListCookie : +pageListCookie : $thisOpts.options.pageSize;
    	         //searchText
        		 $thisOpts.options.searchText = searchTextCookie ? searchTextCookie : '';

    	         if (columnsCookie && columnsCookie != 'undefined') {
    	            $.each($thisOpts.columns, function (i, column) {
    ...
    BootstrapTable.prototype.onSort = function () {
        ...
        if(this.options.cookie)
        setServerCookie(this.options.cookieUrl,this.options.cookieIdTable);
    };
