...
function nanoGALLERY() {
	...
	*/
	...
	function RetrieveCurWidth() {
		...
		if(gO.widthSmall > 0 && vpW < gO.widthSmall) {
			return 'xs';
		}
		if(gO.widthMedium > 0 && vpW < gO.widthMedium) {
			return 'sm';
		}
		if(gO.widthLarge > 0 && vpW < gO.widthLarge) {
			return 'me';
		}
		if(gO.widthXLarge > 0 && vpW < gO.widthXLarge) {
			return 'la';
		}
	...
	function PicasaProcessItems(albumIdx, processLocationHash, imageID, setLocationHash) {
		...
		if(gI[albumIdx].GetID() == 0) {
			...
			// photos
			var opt = '';
			if(typeof gI[albumIdx].customData.authkey !== 'undefined') {
				opt = gI[albumIdx].customData.authkey;
			}
	...
	function SmugmugParseData(albumIdx, data, kind) {
		...
		if(kind == 'album') {
			...
			jQuery.each(data.Albums, function(i, data) {
				...
				var ok = true;
				if(!CheckAlbumName(itemTitle, 'noID')) {
					ok = false;
				}
	...
	function nanoConsoleLog(msg) {
		if(window.console) {
			console.log('nanoGALLERY: ' + msg + ' [' + g_baseEltID + ']');
		}
	};
