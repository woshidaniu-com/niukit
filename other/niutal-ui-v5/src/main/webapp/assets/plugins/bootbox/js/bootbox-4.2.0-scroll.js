  ...
  exports.dialog = function(options) {
    ...
    function _getActualWidth(){
		...
		if(_width >= _windowWidth){
			_realWidth = _windowWidth - options.offsetWidth;
    ...
    function _getDimension(){
    	...
    	// 弹出层内容区域允许的最小高度;为防止计算的结果过于小，需设置最小阀值
    	var _minHeight = Math.max( options.minHeight , Math.min(_contentHeight , _maxHeight ) )
    	...
    	return {
    		...
    		"overflow"			: (_contentHeight > _maxHeight),
    		"top"				: function(){
    			if(_isSmallModel()){
    ...
    function _initCustomScrollbar(){
    	// 扩展增加模拟滚动条支持
    	if(options.customScrollbar && $.fn.mCustomScrollbar && !bootbox_body.data("mCS")){
    		//模拟滚动条
    		bootbox_body.mCustomScrollbar({
    		...
    		bootbox_body.mCustomScrollbar("update");
    	}
    }
    ...
    dialog.on("hide.bs.modal", function(e) {
    	...
    	// 扩展增加模拟滚动条支持
    	if(options.customScrollbar && $.fn.mCustomScrollbar && bootbox_body.data("mCS")){
    		bootbox_body.mCustomScrollbar("destroy");
    	}
    ...
    dialog.on("resize.bs.modal", function(event) {
    	...
    	if(options.customScrollbar && $.fn.mCustomScrollbar && bootbox_body.data("mCS")){
        	...
        	// 超出运行高度范围
    		if(_dimension["overflow"] ){
    			// 获取bootbox弹出,解决弹窗兼容问题
		    	var mCSB_container = bootbox_body.find(".mCSB_container");
		    	console.log(mCSB_container);
    			if(mCSB_container.size() == 1){
    				var _realHeight = ($.fn.actual ? mCSB_container.actual("outerHeight") : mCSB_container.outerHeight(true)) || mCSB_container.height();
    				
    				console.log("_realHeight： " + _realHeight );
    				console.log("_maxHeight： " + _dimension["maxHeight"] );
    			}
    			
    			bootbox_body.mCustomScrollbar("update");
    		}else{
        		bootbox_body.css('max-height', "none");
    			bootbox_body.mCustomScrollbar("destroy");
    		}
    ...
    dialog.on("resized.bs.modal", function(event) {
		...
		if(!_isSmallModel()){
    		bootbox_body.animate({ 'max-height': _dimension["maxHeight"] }, "fast" );
    	}
    ...
    dialog.on("show.bs.modal", function() {
    	...
    	// 扩展增加模拟滚动条支持
    	if(options.customScrollbar && $.fn.mCustomScrollbar ){
    		// 计算尺寸
        	var _dimension = _getDimension();
        	// 超出运行高度范围
    		if(_dimension["overflow"] ){
    			//设置最大高度
        		bootbox_body.css('max-height', _dimension["maxHeight"] );
    		}
    	}
