...
public class JXLSConfigUtils extends ConfigUtils {
	
	private static ThreadLocal<Properties> configCached = new ThreadLocal<Properties>();  
	public	static String DEFAULT_LOCATION = "fastjxls.properties";
	...
	public static Properties getConfig(String location) {
		if (configCached.get() == null) {
			configCached.set(ConfigUtils.getProperties(JXLSConfigUtils.class, location));
        }
