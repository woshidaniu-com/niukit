package com.fastpdf.xhtmlrender.utils;
...
//	private XmlMapper xmlMapper;
	...
	public JacksonBinder(Include inclusion) {
		this.mapper = new ObjectMapper();
		// 设置输出包含的属性
//		this.mapper.getSerializationConfig().withSerializationInclusion(inclusion);
		// 设置输入时忽略JSON字符串中存在而Java对象实际没有的属性
		this.mapper.getDeserializationConfig().with(MapperFeature.IGNORE_DUPLICATE_MODULE_REGISTRATIONS,false);
...
//						false);
	...
	public void setDateFormat(String pattern) {
		if (ObjectUtils.isNotBlank(pattern)) {
			...
			this.mapper.getDeserializationConfig().with(df);
		}
	}
