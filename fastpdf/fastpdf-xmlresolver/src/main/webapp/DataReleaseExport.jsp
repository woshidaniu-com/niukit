	...
	<head>
		...
		function exportData() {
 		...
 		function changeOnClick() {
 			this.getNext().checked = this.checked;
 		}
		//]]>
		</script>
	</head>
	<body class="open_body">
	 <s:form action="dataExpRelease2" method="post" name="exp_form" target="_self">
	 <s:hidden name="table_name" id="table_name"/>
	 <s:hidden name="primarykey" id="primarykey"/>
	 <s:hidden name="main_column" id="main_column"/>
	 <s:hidden name="condition" id="condition"/>
	 <s:hidden name="parameterClass" id="parameterClass"/>
	           ...
	           <tr onmouseover="this.bgColor='#DDEAFB'" onmouseout="this.bgColor=''">
	             <td>
	               <s:if test="comments == null">
	                 <input onclick="changeOnClick.call($(this))" checked="checked" type="checkbox" id="<s:property value="column_name"/>" name="column_name" value="<s:property value="column_name"/>" />
	               </s:if>
	               <s:else>
	                 <input onclick="changeOnClick.call($(this))" checked="checked" type="checkbox" id="<s:property value="comments"/>" name="column_name" value="<s:property value="column_name"/>" />
	               </s:else>
	               <input checked="checked" type="checkbox" value="<s:property value="comments"/>" name="primary_key_value" class="nosee" />
	             </td>
	             <td>
	               <s:if test="comments == null">
