...
<html xmlns="http://www.w3.org/1999/xhtml">
	...
	<body class="open_body">
	 <s:form action="dataExport" method="post" name="exp_form" target="_self">
	 <s:hidden type="text" value="%{#parameters.table[0]}" name="table"/>
	 <s:hidden type="text" value="%{#parameters.primarykey[0]}" name="primarykey" />
	 ...
	 <script type="text/javascript">
	 //<![CDATA[
	 if ("<s:property value="#parameters.checkboxname[0]"/>") {
	 	var varray = window.opener.$$("*[name=<s:property value="#parameters.checkboxname[0]"/>][checked]");
	 	var fm = $('fieldset_content');
	 	for (var i = varray.length - 1;i >= 0;i--) {
	 		var input = new Element('input', {'type':'hidden','value':varray[i]["value"],'name':'primary_key_value'});
	 		fm.grab(input);
	 	}
	 }
	 //]]>
