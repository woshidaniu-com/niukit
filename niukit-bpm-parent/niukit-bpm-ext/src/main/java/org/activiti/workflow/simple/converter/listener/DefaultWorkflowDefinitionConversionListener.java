...
public class DefaultWorkflowDefinitionConversionListener implements WorkflowDefinitionConversionListener {
	...
	private static final String START_EVENT_NAME = "开始";

	private static final String END_EVENT_ID = "end";
	private static final String END_EVENT_NAME = "结束";
	...
	public void afterStepsConversion(WorkflowDefinitionConversion conversion) {
		...
		sequenceFlow.setConditionExpression("${" + TaskDecisionFormProperyHelper.DEFAULT_DECISION_PROPERTY_NAME + "=='PASS'}");
		...
		for (FlowNode flowNode : process.findFlowElementsOfType(FlowNode.class)) {
			List<SequenceFlow> incomingSequenceFlow = sequenceFlowMapping.getIncomingSequenceFlowMapping()
					.get(flowNode.getId());
			if (incomingSequenceFlow != null) {
				flowNode.setIncomingFlows(incomingSequenceFlow);
			}

			List<SequenceFlow> outgoingSequenceFlow = sequenceFlowMapping.getOutgoingSequenceFlowMapping()
					.get(flowNode.getId());
	...
	@Override
	public void beforeStepConversion(WorkflowDefinitionConversion conversion, StepDefinitionConverter stepConverter, StepDefinition step) {
		// TODO Auto-generated method stub

	}

	@Override
	public void afterStepConversion(WorkflowDefinitionConversion conversion, StepDefinitionConverter stepConverter, StepDefinition step) {
		Process process = conversion.getProcess();
		if(step instanceof CustomHumanStepDefinition){
			Deque<SequenceFlow> additionalNOPASSSequenceFlow = ((CustomHumanStepDefinition)step).getAdditionalNOPASSSequenceFlow();
			if(additionalNOPASSSequenceFlow.size() > 0){
				while(!additionalNOPASSSequenceFlow.isEmpty()){
					SequenceFlow pop = additionalNOPASSSequenceFlow.pop();
					EndEvent endEvent = new EndEvent();
					endEvent.setId(conversion.getUniqueNumberedId(END_EVENT_ID));
					endEvent.setName(END_EVENT_NAME);
					pop.setTargetRef(endEvent.getId());
					process.addFlowElement(pop);
					process.addFlowElement(endEvent);
				}
			}
		}
