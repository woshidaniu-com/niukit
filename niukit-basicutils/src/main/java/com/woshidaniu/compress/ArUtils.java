...
/**
 * 
 * @className	： ArArchiveUtils
 * @description	： Apache commons compress AR打包
 * @author 		： kangzhidong
 * @date		： Aug 10, 2015 4:51:52 PM
 */
public abstract class ArUtils extends CompressUtils{
	...
	public static void compress(File srcFile, File destFile,String basePath) throws IOException {
		...
		try {
			output = new ArArchiveOutputStream(new BufferedOutputStream(new FileOutputStream(destFile), DEFAULT_BUFFER_SIZE));
			//压缩
	...
	public static void decompress(File srcFile, File destDir) throws IOException {  
        ...
        try {  
            input = new ArArchiveInputStream(new BufferedInputStream(new FileInputStream(srcFile), DEFAULT_BUFFER_SIZE));  
            //解压
                ...
                if (entry.isDirectory()) {  
                    ...
                    try {  
                        ...
                        bos = new BufferedOutputStream(new FileOutputStream(  
                                new File(destDir, entry.getName())), DEFAULT_BUFFER_SIZE);  
                        int len;  
