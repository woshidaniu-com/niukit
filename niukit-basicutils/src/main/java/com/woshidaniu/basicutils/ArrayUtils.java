...
public abstract class ArrayUtils extends org.apache.commons.lang.ArrayUtils {
	...
	public static byte sk(byte[] array) {
		...
		BigDecimal x = new BigDecimal(avg(array)).subtract(new BigDecimal(
				me(array)));
	...
	public static byte σ(byte[] array) {
		...
		exceptionCheck(array);
		// Finds and returns σ
		return new Double(Math.sqrt(new Double(s(array)).doubleValue()))
				.byteValue();
	}
	...
	/**
	 * <p>
	 * Returns the s^2 in an array.
	 * </p>
	 * <p>
	 * s^2=[(x1-x)^2 +(x2-x)^2 +......(xn-x)^2]/n 　　(x为平均数) 　　 例如：4,8,6,2，方差为5.
	 * 　
	 * </p>
	 * 
	 * @param array
	...
	public static byte s(byte[] array) {
		...
		return new BigDecimal(sum).divide(new BigDecimal(array.length))
				.byteValue();
		// sum/array.length;
	}
	...
	/**
	 * <p>
	 * Returns the Me in an array.
	 * </p>
	 * <p>
	 * 一般来说，一组数据中，出现次数最多的数就叫这组数据的众数。 　　例如：1，2，3，3，4的众数是3。
	 * 　　但是，如果有两个或两个以上个数出现次数都是最多的，那么这几个数都是这组数据的众数。 　　例如：1，2，2，3，3，4 的众数是2和3。
		...
		if (array.length > 1) {
			...
			if (!eq) {
				...
				for (Object object : tempList) {
					r_mo[maxIndex] = new Float(String.valueOf(object))
							.byteValue();
					maxIndex++;
				}
			}
	...
	public static int[] mo(int[] array) {
		...
		if (array.length > 1) {
			...
			if (!eq) {
				...
				int maxIndex = 0;
				for (Object object : tempList) {
					r_mo[maxIndex] = Integer.valueOf(String.valueOf(object))
							.intValue();
					maxIndex++;
				}
			}
	...
	public static double[] mo(double[] array) {
		...
		if (array.length > 1) {
			...
			if (!eq) {
				...
				int maxIndex = 0;
				for (Object object : tempList) {
					r_mo[maxIndex] = Double.valueOf(String.valueOf(object))
							.doubleValue();
					maxIndex++;
				}
			}
	...
	public static float[] mo(float[] array) {
		...
		if (array.length > 1) {
			...
			if (!eq) {
				...
				int maxIndex = 0;
				for (Object object : tempList) {
					r_mo[maxIndex] = Float.valueOf(String.valueOf(object))
							.floatValue();
					maxIndex++;
				}
			}
	...
	public static long[] mo(long[] array) {
		...
		if (array.length > 1) {
			...
			if (!eq) {
				...
				int maxIndex = 0;
				for (Object object : tempList) {
					r_mo[maxIndex] = Long.valueOf(String.valueOf(object))
							.longValue();
					maxIndex++;
				}
			}
	 ...
	 * </p>
	 * <p>
	 * 当一组数字的个数是偶数的时候，是一组数字顺序排列后中间2位的数字的平均值
	 * </p>
	 * <p>
	 * 当一组数字的个数是奇数的时候，是一组数字顺序排列后中间1位的数字
	 * </p>
	 * 
	 * @param array
	 *            an array, must not be null or empty
	...
	public static byte me(byte[] array) {
		...
		byte v_mo = 0;
		if (array.length % 2 == 0) {
			v_mo = new BigDecimal(array[array.length / 2 - 1]).add(
					new BigDecimal(array[array.length / 2])).divide(
					new BigDecimal(2)).byteValue();
		} else {
	...
	public static short me(short[] array) {
		...
		short v_me = 0;
		if (array.length % 2 == 0) {
			v_me = new BigDecimal(array[array.length / 2 - 1]).add(
					new BigDecimal(array[array.length / 2])).divide(
					new BigDecimal(2)).shortValue();
		} else {
