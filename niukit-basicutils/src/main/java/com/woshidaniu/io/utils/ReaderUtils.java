...
public abstract class ReaderUtils {
    ...
    public static StringBuffer readReader(Reader in, int len) throws IOException {
        StringBuffer ret = new StringBuffer();
    ...
    public static StringBuffer readInputStream(InputStream in, String charset) throws IOException {
        return readReader(new InputStreamReader(in, charset));
    }

    public static StringBuffer readInputStream(InputStream in, String charset, int len) throws IOException {
        return readReader(new InputStreamReader(in, charset), len);
    }

    public static StringBuffer readInputStream(InputStream in) throws IOException {
        return readReader(new InputStreamReader(in));
    }

    public static StringBuffer readInputStream(InputStream in, int len) throws IOException {
	...
	public static char[] readerBytes(byte[] bytes) throws IOException {
		...
		int a = 0;
		try {
			DataInputStream in3 = new DataInputStream(new ByteArrayInputStream(bytes));
			while (true) {
	...
	public static StringBuffer readFile(String filepath) throws IOException, URISyntaxException {
		return readFile(new File(LocationUtils.getExtendResourceAsURL(filepath).toURI()));
    }
