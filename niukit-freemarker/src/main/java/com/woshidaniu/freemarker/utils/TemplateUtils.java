package com.woshidaniu.freemarker.utils;

import java.io.File;
import java.io.IOException;

import com.woshidaniu.freemarker.model.LookupModel;

import freemarker.cache.StringTemplateLoader;
import freemarker.template.Configuration;
import freemarker.template.Template;

/**
 * 
 *@类名称:TemplateUtils.java
 *@类描述：
 *@创建人：kangzhidong
 *@创建时间：Dec 29, 2015 11:13:49 AM
 *@修改人：
 *@修改时间：
 *@版本号:v1.0
 */
public abstract class TemplateUtils {

	/**
	 * String文本模板加载器
	 */
	private static StringTemplateLoader sTmpLoader = new StringTemplateLoader();

	/**
	 * 
	 *@描述：获取以string文本为模板的FreeMarker模板对象
	 *@创建人:kangzhidong
	 *@创建时间:Dec 29, 20154:06:03 PM
	 *@param templateName	：模板名称
	 *@param templateSource	：模板内容
	 *@return
	 *@throws IOException
	 *@修改人:
	 *@修改时间:
	 *@修改描述:
	 */
	public static Template getStringTemplate(String templateName,String templateSource) throws IOException{
		// 创建 Configuration 实例
		Configuration configuration = ConfigurationUtils.getDefaultConfiguration();
		// 获取模板
		return TemplateUtils.getStringTemplate(configuration,templateName,templateSource);
	}
	
	/**
	 * 
	 *@描述：获取以string文本为模板的FreeMarker模板对象
	 *@创建人:kangzhidong
	 *@创建时间:Dec 29, 20154:05:59 PM
	 *@param configuration	： Configuration对象
	 *@param templateName	：模板名称
	 *@param templateSource ：模板内容
	 *@return
	 *@throws IOException
	 *@修改人:
	 *@修改时间:
	 *@修改描述:
	 */
	public static Template getStringTemplate(Configuration configuration,String templateName,String templateSource) throws IOException{
		//添加字符内容模板
		sTmpLoader.putTemplate(templateName, templateSource);
		//设置模板加载方式
		configuration.setTemplateLoader(sTmpLoader);
		// 获取模板
		return configuration.getTemplate(templateName);
	}
	
	/**
	 * 
	 *@描述：以文件路径模板装载方式获取【指定文件目录】下的FreeMarker模板对象
	 *@创建人:kangzhidong
	 *@创建时间:Dec 29, 20154:04:33 PM
	 *@param templateDir
	 *@param templateName
	 *@return
	 *@throws IOException
	 *@修改人:
	 *@修改时间:
	 *@修改描述:
	 */
	public static Template getFileTemplate(File templateDir,String templateName) throws IOException{
		// 创建 Configuration 实例
		Configuration configuration = ConfigurationUtils.getConfiguration(templateDir);
		// 获取模板
		return configuration.getTemplate(templateName);
	}
	
	/**
	 * 
	 *@描述：以classpath 模板装载方式获取【classpath根目录】下的FreeMarker模板对象
	 *@创建人:kangzhidong
	 *@创建时间:Dec 29, 20153:56:11 PM
	 *@param clazz			： class对象
	 *@param templateName	：模板名称
	 *@return
	 *@throws IOException
	 *@修改人:
	 *@修改时间:
	 *@修改描述:
	 */
	public static Template getClassRootTemplate(Class<?> clazz, String templateName) throws IOException{
		// 创建 Configuration 实例
		Configuration configuration = ConfigurationUtils.getClassRootConfiguration(clazz);
		// 获取模板
		return configuration.getTemplate(templateName);
	}
	
	/**
	 * 
	 *@描述：以classpath 模板装载方式获取【指定class所在目录】下的FreeMarker模板对象
	 *@创建人:kangzhidong
	 *@创建时间:Dec 29, 20153:56:11 PM
	 *@param clazz			： class对象
	 *@param pathPrefix		：模板文件所在目录
	 *@param templateName	：模板名称
	 *@return
	 *@throws IOException
	 *@修改人:
	 *@修改时间:
	 *@修改描述:
	 */
	public static Template getClasspathTemplate(Class<?> clazz, String templateName) throws IOException{
		// 创建 Configuration 实例
		Configuration configuration = ConfigurationUtils.getClasspathConfiguration(clazz);
		// 获取模板
		return configuration.getTemplate(templateName);
	}
	
	/**
	 * 
	 *@描述：以classpath 模板装载方式获取【指定classpath目录】下的FreeMarker模板对象
	 *@创建人:kangzhidong
	 *@创建时间:Dec 29, 20153:56:11 PM
	 *@param clazz			： class对象
	 *@param pathPrefix		：模板文件所在目录
	 *@param templateName	：模板名称
	 *@return
	 *@throws IOException
	 *@修改人:
	 *@修改时间:
	 *@修改描述:
	 */
	public static Template getClasspathTemplate(Class<?> clazz, String pathPrefix,String templateName) throws IOException{
		// 创建 Configuration 实例
		Configuration configuration = ConfigurationUtils.getConfiguration(clazz,pathPrefix);
		// 获取模板
		return configuration.getTemplate(templateName);
	}
	
	public static Template getClasspathTemplate(ClassLoader classLoader, String pathPrefix , String templateName) throws IOException{
		// 创建 Configuration 实例
		Configuration configuration = ConfigurationUtils.getConfiguration(classLoader,pathPrefix);
		// 获取模板
		return configuration.getTemplate(templateName);
	}
	
	public static Template getLookupTemplate(LookupModel lookup, String pathPrefix,String templateName) throws IOException{
		// 创建 Configuration 实例
		Configuration configuration = ConfigurationUtils.getLookupConfiguration(lookup,pathPrefix);
		// 获取模板
		return configuration.getTemplate(templateName);
	}
	
	public static Template getSpringClasspathTemplate(String resource) throws IOException{
		// 创建 Configuration 实例
		Configuration configuration = ConfigurationUtils.getSpringClasspathConfiguration();
		// 获取模板
		return configuration.getTemplate(resource);
	}
	
}
